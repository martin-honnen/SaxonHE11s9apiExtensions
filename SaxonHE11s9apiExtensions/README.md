...
The basic usage is to to install the NuGet package IKVM.Maven.Sdk to be able to pull in the Saxon HE 11 (e.g. 11.5) and the XmlResolver code it uses directly from Maven:
```
  <ItemGroup>
...
Then you are ready to write .NET 6 or .NET framework 4.8 code against the Saxon 11 s9api API, helped by this extension library to not have to use most Java classes for input/output/URIs/URLs but to be able to use relevant .NET classes:
